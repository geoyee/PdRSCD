...
class _PAMBlock(nn.Layer):
    ...
    def forward(self, input):
        ...
        def func(value_local, query_local, key_local):
            ...
            value_local = value_local.contiguous().reshape([batch_size_new, self.value_channels, -1])
            query_local = query_local.contiguous().reshape([batch_size_new, self.key_channels, -1])
            query_local = query_local.transpose((0, 2, 1))
            key_local = key_local.contiguous().reshape([batch_size_new, self.key_channels, -1])
            sim_map = paddle.bmm(query_local, key_local)
            sim_map = (self.key_channels ** -.5) * sim_map
            sim_map = F.softmax(sim_map, axis=-1)
            context_local = paddle.bmm(value_local, sim_map.transpose((0, 2, 1)))
